...
fun main() {
    ...
    println(Solution176962().solution(plans).joinToString(" "))
...
class Solution176962() {
    ...
    fun solution(plans: Array<Array<String>>): Array<String> {
        val plansSize = plans.size

        val arr = Array<Study>(plansSize) { i ->
            plans[i].let { plan ->
                Study(
                    name = plan[0],
                    startTime = plan[1].convertTime(),
                    remainStudyTime = plan[2].toInt()
                )
            }
        }

        arr.sortBy(Study::startTime)

        var nowTime = arr[0].startTime
        val answer: Array<String> = Array<String>(plansSize) { "" }
        var ansIdx = 0

        fun divide(i: Int) {// 비교작업
            if (i == plansSize - 1) {// 현재가 모든계획중 마지막 계획일때
                arr[i].isFinished = true
                answer[ansIdx++] = arr[i].name
                nowTime += arr[i].remainStudyTime
                for (j in i - 1 downTo 0) {//이전 끝내지 않은작업들의 비교연산
                    if (!arr[j].isFinished) {
                        arr[j].isFinished = true
                        answer[ansIdx++] = arr[j].name
                    }
                }
            } else {//현재 진행한 계획이 모든 계획의 끝이 아닐때
                if (nowTime + arr[i].remainStudyTime <= arr[i + 1].startTime) {//현재 계획의 끝나는 시간이 다음계획의 시작시간을 넘지 않을때
                    arr[i].isFinished = true
                    answer[ansIdx++] = arr[i].name
                    nowTime += arr[i].remainStudyTime
                    for (j in i - 1 downTo 0) {//이전 끝내지 않은작업들의 비교연산
                        if (!arr[j].isFinished) {
                            if (nowTime + arr[j].remainStudyTime <= arr[i + 1].startTime) {
                                arr[j].isFinished = true
                                answer[ansIdx++] = arr[j].name
                                nowTime += arr[j].remainStudyTime
                            } else {
                                arr[j].remainStudyTime -= arr[i + 1].startTime - nowTime
                                nowTime = arr[i + 1].startTime
                                return divide(i + 1)
                            }
                        }
                    }
                    ...
                    arr[i].remainStudyTime -= arr[i + 1].startTime - nowTime
                    nowTime = arr[i + 1].startTime
                }
                divide(i + 1)
            }
