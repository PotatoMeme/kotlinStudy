...
class Solution150370 {
    fun solution(today: String, terms: Array<String>, privacies: Array<String>): List<Int> {
        ...
        val sum1 = y * 12 * 28 + m * 28 + d
        val types = IntArray(26)
        terms.forEach {
            val (type, value) = it.split(" ")
            types[type[0] - 'A'] = value.toInt()
        }
        privacies.forEachIndexed { index, s ->
            val (date, type) = s.split(" ")
            val (it_y, it_m, it_d) = date.split(".").map { it.toInt() }
            val sum2 = it_y * 12 * 28 + (it_m + types[type[0] - 'A']) * 28 + it_d
            if (sum1 >= sum2) answer.add(index + 1)
        }
        return answer
    }
}