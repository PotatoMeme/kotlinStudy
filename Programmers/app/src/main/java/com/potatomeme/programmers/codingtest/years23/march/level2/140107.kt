...
class Solution140107() {
    ...
    // 1안
    // 피타고라스 방정식을 이용
    // 임의의 변수 i와 j를 이용하여 (i*k)^2 + (j*k)^2 <= (d)^2를 만족하는 i와 j의 조합 개수를 구하자
    // (j*k)^2 <= (d*k)^2 - (i*k)^2
    // j^2 <= d^2 / k^2 - i^2
    // 1,000,000 ^ 2 = 1_000_000_000_000
    // Int.MAX_VALUE = 2_147_483_647

    fun solution(k: Int, d: Int): Long {
        var cnt: Long = 0
        var i = 0L
        val n : Long = (d.toLong() * d) / (k.toLong() * k)

        while (true) {
            val j_pow_max = n - i*i
            if (j_pow_max < 0) break
            cnt += (sqrt(j_pow_max.toDouble()).toInt() + 1)
            i++
        }
