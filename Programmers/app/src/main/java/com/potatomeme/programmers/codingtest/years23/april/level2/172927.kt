...
class Solution172927() {
    fun solution(picks: IntArray, minerals: Array<String>): Int {
        val arr = IntArray(minerals.size) { i ->
            when (minerals[i]) {
                "diamond" -> 3
                "iron" -> 2
                "stone" -> 1
        ...
        fun solve(idx: Int, statusPick: Int, usePick: Int, remainPick: Int, sum: Int) {
            if (idx == arr.size || (remainPick == 0 && statusPick == 0)) {
                if (sum < max) max = sum
                return
            }
            ...
            if (statusPick == 0) {
                for (i in 0..2) {
                    if (picks[i] > 0) {
                        picks[i]--
                        solve(idx + 1, 4, i, remainPick - 1, sum + when (i) {
                            0 -> 1
                            1 -> if (arr[idx] > 2) 5 else 1
                            2 -> if (arr[idx] > 2) 25 else if (arr[idx] > 1) 5 else 1
                            else -> 0
                        })
                        picks[i]++
                    }
                ...
                solve(idx + 1, statusPick - 1, usePick, remainPick, sum + when (usePick) {
                    0 -> 1
                    1 -> if (arr[idx] > 2) 5 else 1
                    2 -> if (arr[idx] > 2) 25 else if (arr[idx] > 1) 5 else 1
                    else -> 0
