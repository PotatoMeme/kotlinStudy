...
class Solution181890() {
    fun solution(str_list: Array<String>): Array<String> {
        for ((index: Int, str: String) in str_list.withIndex()) {
            when (str) {
                "l" -> return str_list.copyOfRange(0, index)
                "r" -> return str_list.copyOfRange(index + 1, str_list.size)
            }
        }
        return emptyArray()
    }
