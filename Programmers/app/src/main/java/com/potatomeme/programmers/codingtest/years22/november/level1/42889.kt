...
class Solution42889 {
    fun solution(N: Int, stages: IntArray): IntArray {
        ...
        val list_clear = MutableList(N + 1) { 0 }
        stages.forEach {
            for (i in 0 until it) {
                list_clear[i] += 1
            }
        ...
        for (i in 0 until N) map[i + 1] =
            if (list_fail[i] == 0) 0.0 else list_fail[i].toDouble() / list_clear[i]
    ...
    fun user1solution(N: Int, stages: IntArray): IntArray {
