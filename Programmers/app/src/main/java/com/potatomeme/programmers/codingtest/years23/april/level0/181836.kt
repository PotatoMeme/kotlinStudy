...
class Solution181836() {
    fun solution(picture: Array<String>, k: Int): Array<String> {
        val answer: Array<String> = Array(picture.size * k) { "" }

        picture.forEachIndexed { index, s ->
            answer[index * k] = buildString {
                s.forEach { c: Char ->
                    repeat(k) { append(c) }
                }
            }
            for (i in 1 until k) {
                answer[index * k + i] = answer[index * k]
            }
        }
