...
class Solution118666 {
    fun solution(survey: Array<String>, choices: IntArray): String {
        val resultArr : IntArray = IntArray('Z' - 'A' + 1)
        choices.forEachIndexed { index, i ->
            if (i > 4){
                resultArr[survey[index][1] - 'A'] += i - 4
            }else{
                resultArr[survey[index][0] - 'A'] += 4 - i
            }
        }

        return buildString {
            if (resultArr['R' - 'A'] >= resultArr['T' - 'A']) append('R') else append('T')
            if (resultArr['C' - 'A'] >= resultArr['F' - 'A']) append('C') else append('F')
            if (resultArr['J' - 'A'] >= resultArr['M' - 'A']) append('J') else append('M')
            if (resultArr['A' - 'A'] >= resultArr['N' - 'A']) append('A') else append('N')
        }
    }
}