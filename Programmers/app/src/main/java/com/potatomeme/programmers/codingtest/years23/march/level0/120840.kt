...
class Solution120840() {
    fun solution(balls: Int, share: Int): Int {
        return solve(balls,0,share,0)
    }
    fun solve(max: Int ,depth:Int,select_max:Int,select : Int):Int{
        if (select_max == select) return 1
        if (max == depth) return 0
        return solve(max,depth+1,select_max,select+1) + solve(max,depth+1,select_max,select)
    }
}