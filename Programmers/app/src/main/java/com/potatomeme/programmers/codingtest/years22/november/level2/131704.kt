...
class Solution {
    fun solution(order: IntArray): Int {
        var stack_sub : Stack<Int> = Stack()
        var stack_main  : Stack<Int> = Stack()
        for (i in order.size downTo 1) stack_main.add(i)

        var order_index = 0
        while (true){
            if (order_index == order.size) break
            if (stack_sub.isNotEmpty()){
                if (order[order_index] == stack_sub.peek()) {
                    stack_sub.pop()
                    order_index++
                    continue
                }
            }
            if (stack_main.isNotEmpty()){
                if (order[order_index] == stack_main.peek()) {
                    stack_main.pop()
                    order_index++
                    continue
                }
            ...
            stack_sub.add(stack_main.pop())
        }
        return order_index
    }//13.43~141.30ms 최대 메모리143

    fun user1Solution(order: IntArray): Int {
        val bozo = Stack<Int>()
        var box = 1
        var index = 0
        ...
        return index
    }//5.08~43.28ms 최대 메모리115

    fun user2Solution(order: IntArray): Int {
        var answer: Int = 0
        ...
        for (o in order) {
            when(o) {
                in 1 until num -> {
                    if (stack.isNotEmpty() && stack.peek() == o) {
                        stack.pop()
                        answer++
                        // println("$num $answer")
                    } else {
