...
class Solution181873() {
    fun solution1(my_string: String, alp: String): String {
        val x = 'A' - 'a'
        val c = alp[0]
        return my_string.map { if (it == c) it + x else it }.joinToString("")
    }

    fun solution2(my_string: String, alp: String): String {
        val x = 'A' - 'a'
        val c = alp[0]
        return my_string.replace(c, c + x)
    }
}
