...
class Solution132265 {
    fun solution(topping: IntArray): Int {
        var countSave = mutableMapOf<Int, Int>()
        ...
        topping.forEach {
            ...
            input.add(it)
            println("${input.size}  ${countSave.size}")
            if (input.size == countSave.size) count++
        }
        return count
    }// 12 ~ 350ms mutableMap이 너무큼 다른 collection으로 바꾸는게 좋을것 같음

    fun user1Solution(topping: IntArray): Int {
        var answer: Int = 0
        val left = IntArray(topping.size+1)
        val right = IntArray(topping.size+1)
        val toppingSet = mutableSetOf<Int>()

        for(i in 1 until topping.size+1){
            if(toppingSet.contains(topping[i-1])){
                left[i] = left[i-1]
            }else {
                left[i] = left[i-1] + 1
                toppingSet.add(topping[i-1])
            }
        }
        toppingSet.clear()
