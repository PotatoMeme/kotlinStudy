...
class Solution169198() {
    fun solution(m: Int, n: Int, startX: Int, startY: Int, balls: Array<IntArray>): IntArray {
        val answer = IntArray(balls.size)

        balls.forEachIndexed { index, after ->
            var min = Int.MAX_VALUE

            if (!(startX == after[0] && startY < after[1]))
                getDistance(startX, startY, after[0], n + (n - after[1]))
                    .let { if (min > it) min = it }//위로 원쿠션

            if (!(startY == after[1] && startX < after[0]))
                getDistance(startX, startY, m + (m - after[0]), after[1])
                    .let { if (min > it) min = it }//오른쪽으로 원쿠션

            if (!(startX == after[0] && startY > after[1]))
                getDistance(startX, startY, after[0], -after[1])
                    .let { if (min > it) min = it }//아래쪽으로 원쿠션

            if (!(startY == after[1] && startX > after[0]))
                getDistance(startX, startY, -after[0], after[1])
                    .let { if (min > it) min = it }//왼쪽으로 원쿠션
    ...
    private fun getDistance(x: Int, y: Int, afterX: Int, afterY: Int): Int {
        return (afterX - x).pow() + (afterY - y).pow()
    }
}