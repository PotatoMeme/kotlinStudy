package com.potatomeme.programmers.codingtest.years22.december.level2
...
class Solution131130 {
    fun solution(cards: IntArray): Int {
        var boxSize = mutableListOf<Int>()
        var isNotSelect = BooleanArray(cards.size) { true }
        cards.forEachIndexed { index, i ->
            if (isNotSelect[index]) {
                isNotSelect[index] = false
                var count = 1
                var check = i - 1
                while (isNotSelect[check]) {
                    count++
                    isNotSelect[check] = false
                    check = cards[check] - 1
                }
                boxSize.add(count)
            }
        }
        return if (boxSize.size == 1) 0 else {
            boxSize.sortDescending()
            boxSize[0] * boxSize[1]
        }
    }//0.04 ~ 10.04ms
}