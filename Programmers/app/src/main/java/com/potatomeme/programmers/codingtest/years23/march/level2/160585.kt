...
class Solution160585() {
    ...
    // val x_cnt
    // * 1규칙
    // x_cnt in o_cnt - 1 .. o_cnt => true
    // * 2규칙
    // 승리라인은 판에서 한번 밖에 있을수 없다(X)
    // 승리라인은 겹쳐있는 경우 여러개(최대 2개)
    // o_cnt는 최대 5
    // x_cnt는 최대 4
    // O의 승리 라인이 2개인경우에도 최대 5개이므로 무조건 겹쳐있는승리다

    // 승리확인 알고리즘
    // 나올수있는 경우는 총  8가지
    // 재귀로 풀수 있지만 8가지경우를 모두 확인을 하는것이 더빠를것 같음
    ...
    fun solution(board: Array<String>): Int {
        ...
        if (xCnt !in oCnt - 1..oCnt) return 0
        var winOCnt = 0
        var winXCnt = 0

        if ((board[0][0] != '.') && (board[0][0] == board[0][1] && board[0][1] == board[0][2])) {
            if (board[0][0] == 'O') winOCnt++ else winXCnt++
        }
        if ((board[1][0] != '.') && (board[1][0] == board[1][1] && board[1][1] == board[1][2])) {
            if (board[1][0] == 'O') winOCnt++ else winXCnt++
        }
        if ((board[2][0] != '.') && (board[2][0] == board[2][1] && board[2][1] == board[2][2])) {
            if (board[2][0] == 'O') winOCnt++ else winXCnt++
        }
        if ((board[0][0] != '.') && (board[0][0] == board[1][0] && board[1][0] == board[2][0])) {
            if (board[0][0] == 'O') winOCnt++ else winXCnt++
        }
        if ((board[0][1] != '.') && (board[0][1] == board[1][1] && board[1][1] == board[2][1])) {
            if (board[0][1] == 'O') winOCnt++ else winXCnt++
        }
        if ((board[0][2] != '.') && (board[0][2] == board[1][2] && board[1][2] == board[2][2])) {
            if (board[0][2] == 'O') winOCnt++ else winXCnt++
        }
        ...
        return if (winXCnt == 1) {//X가 승리
            if (winOCnt == 0 && xCnt == oCnt) 1 else 0
        } else if (winOCnt > 0) {//O가 승리
            if (xCnt < oCnt) 1 else 0
        } else {//무승부
            1
        }
    }
