...
class Solution142085() {
    ...
    fun solution(n: Int, k: Int, enemy: IntArray): Int {
        ...
        fun solve(depth: Int, remain: Int, skillCount: Int) {
            if (depth == enemySize) {
                maxDepth = depth
                return
            }
            if (skillCount != k) {//무적권을 다쓰지 않은경우
                solve(depth + 1, remain, skillCount + 1)
            }

            if (remain < enemy[depth]) {
                if (maxDepth < depth) maxDepth = depth
            } else {
                solve(depth + 1, remain - enemy[depth], skillCount)
            }
        }
    ...
    //2안
    //Priority Queue 이용
    fun solution2(n: Int, k: Int, enemy: IntArray): Int {
        val priorityQueue = PriorityQueue<Int>()
        var remain = n

        for (i in enemy.indices) {
            priorityQueue.add(enemy[i])
            if (priorityQueue.size > k) remain -= priorityQueue.poll()
