...
class Solution17679 {
    fun solution(
        n: Int,//폭,board[0].length
        m: Int,//높이,board.size
        board: Array<String>,
    ): Int {
        val charArrays = board.map { it.toCharArray() }
        while (true) {
            val arrayList: ArrayList<Pair<Int, Int>> = arrayListOf()
            for (i in 0 until m - 1) {
                for (j in 0 until n - 1) {
                    if (charArrays[i][j] != '0' && charArrays[i][j] == charArrays[i][j + 1] && charArrays[i][j] == charArrays[i + 1][j] && charArrays[i][j] == charArrays[i + 1][j + 1]) {
        ...
        return charArrays.fold(0) { acc, charArray -> acc + charArray.count { it == '0' } }
    }
}