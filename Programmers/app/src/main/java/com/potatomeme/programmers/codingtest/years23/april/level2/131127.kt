...
class Solution131127() {
    ...
    fun solution(want: Array<String>, number: IntArray, discount: Array<String>): Int {
        val map = mutableMapOf<String, Int>().apply {
            want.forEachIndexed { index, s -> this[s] = index }
        }

        val arr = IntArray(want.size)
        fun isCorrect(): Boolean {
            for (i in number.indices) {
                if (arr[i] != number[i]) return false
            }
            return true
        }
        ...
        for (i in discount.indices) {
            map[discount[i]]?.let { arr[it]++ }
            if (i > 8) {
                if (i > 9) map[discount[i - 10]]?.let { arr[it]-- }
                if (isCorrect()) cnt++
            }
        }
