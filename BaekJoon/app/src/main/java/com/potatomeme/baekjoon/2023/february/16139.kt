...
//1 초 (추가 시간 없음)	256 MB	11976	3194	2598	29.399%
//문제
...
fun main(args: Array<String>) = with(System.`in`.bufferedReader()) {
    ...
    val s = readLine()
    val len = s.length
    val arr = Array(len + 1) { IntArray(26) }
    for (i in 1..len) {
        arr[i] = arr[i - 1].copyOf()
        arr[i][s[i - 1] - 'a'] += 1
    }
    val sb = StringBuilder()
    repeat(readLine().toInt()) {
        val st = StringTokenizer(readLine())
        val i = st.nextToken()[0] - 'a'
        val l = st.nextToken().toInt()
        val r = st.nextToken().toInt()
        sb.append(arr[r + 1][i] - arr[l][i]).appendLine()
    }
    print(sb.toString())
}
