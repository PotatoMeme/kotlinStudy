...
//1 초	128 MB	10868	2078	1709	20.028%
//문제
...
fun main()  = with(System.`in`.bufferedReader()) {
    val variableName = readLine()
    val convertedName = convertVariableName(variableName)
    print(convertedName)
}

fun convertVariableName(variableName: String): String {
    return when {
        variableName.matches("[a-z][a-zA-Z]*".toRegex()) -> {
            variableName.replace("([A-Z])".toRegex()){
                "_${it.value.lowercase()}"
            }
        }
        variableName.matches("[a-z][a-z_]*".toRegex()) -> {
            if (variableName.last() == '_' || variableName.contains("__") ) "Error!" else variableName.replace("_[a-z]".toRegex()){
