...
fun parseHTML(html: String) {
    val divPattern = "<div title=\"([^\"]+)\">(.*?)</div>".toRegex()
    val pPattern = "<p>(.*?)</p>".toRegex()
    val tagPattern = "<.*?>".toRegex()

    val divMatches = divPattern.findAll(html)
    for (divMatch in divMatches) {
        val title = divMatch.groupValues[1]
        val divContent = divMatch.groupValues[2]

        println("title : $title")

        val pMatches = pPattern.findAll(divContent)
        for (pMatch in pMatches) {
            val pContent = pMatch.groupValues[1]
            val parsedContent = pContent
                .replace(tagPattern, "")
                .replace(Regex("\\s{2,}"), " ")
            println(parsedContent.trim())
        }
    }
}