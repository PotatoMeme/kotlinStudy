...
//1 초	128 MB	6872	2744	2019	40.477%
//문제
...
fun main() = with(System.`in`.bufferedReader()) {
    print(buildString {
        repeat(readLine().toInt()) {
            val line = readLine()
            appendLine(if (isSlurpy(line)) "YES" else "NO")
        }
    })
}

val regexSlump = "([DE]F+)+G".toRegex()
fun isSlump(str: String): Boolean {
    return regexSlump.matches(str)
}
...
fun isSlimp(str: String): Boolean {
    ...
    if (str[0] == 'A' && str.last() == 'C') {
        return if (str[1] == 'B') {
            isSlimp(str.substring(2, str.length - 1))
        } else {
            isSlump(str.substring(1, str.length - 1))
        }
    }
    return false
}

fun isSlurpy(str: String): Boolean {
    val lastCIndex = str.lastIndexOf('C')
    if (lastCIndex == -1) {
        if (str.startsWith("AH")) return isSlump(str.substring(2))
    } else if (
        isSlimp(str.substring(0, lastCIndex + 1)) &&
        isSlump(str.substring(lastCIndex + 1))
    ) return true
    return false
}