...
fun main() {
    with(System.`in`.bufferedReader()) {
        ...
        val operators = BooleanArray(k) { st.nextToken() == "<" }

        val nums = BooleanArray(10)
        var min = Long.MAX_VALUE
        var max = Long.MIN_VALUE

        fun dfs(depth: Int, current: Long, beforeNum: Int) {
            if (depth > k) {
            ...
            if (operators[depth - 1]) {
                for (i in beforeNum + 1..9) {
                    if (!nums[i]) {
                        nums[i] = true
                        dfs(depth + 1, current * 10 + i, i)
                        nums[i] = false
                    }
                }
            } else {
                for (i in beforeNum - 1 downTo 0) {
                    if (!nums[i]) {
                        nums[i] = true
                        dfs(depth + 1, current * 10 + i, i)
                        nums[i] = false
